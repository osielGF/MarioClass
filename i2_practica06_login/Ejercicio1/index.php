...
<?php
    ...
    //TOTAL DE USUARIOS
    $query = "SELECT COUNT(*) as total_users FROM user WHERE borrado_logico = 0";//Se declara una variable que contendra la cadena de la consulta para obtener el total de usuarios
    ...
    $query = "SELECT *  FROM user WHERE borrado_logico = 0";//Se declara una variable que contendra la cadena de la consulta para obtener todo el contenido de la tabla usuario
    $statement = $pdo->prepare($query);//Declaracion de una variable la cual contendra la varibale que represetnara PDO en el archivo "connection.php" y recibira como parametro la consulta anterior
    $statement->execute();//Se ejecutara la consulta con PDO
...
<html class="no-js" lang="en">
    <head>
        ...
        <title>Practica 06 | Ejercicio N°1</title>
            ...
            <div class="large-12 columns" align="center">
                <div class="section-container tabs" data-section>
                    ...
                    <section class="section">
                        ...
                        <a align="left" href="agregar.php" class="button success" style="border-radius:10px">Agregar Usuario</a>
                         <div class="content" data-slug="panel1">
                            <div class="row">
                                <table>
                                    <thead>
                                        <tr>
                                            ...
                                            <th widht="250">Modificar</th>
                                            <th widht="250">Eliminar</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        ...
                                        ?>
                                            <tr> 
                                                ...
                                                <?php 
                                                    $stat = $user[$i]['status_id'];
                                                    $query = "SELECT * FROM status WHERE id = $stat";
                                                    $statement = $pdo->prepare($query);
                                                    $statement->execute();
                                                    $results = $statement->fetch(); 
                                                ?>

                                                <td><?php echo $user[$i]['status_id']. " - " . $results['name'];?></td>

                                                <?php 
                                                    $usty = $user[$i]['user_type_id'];
                                                    $query = "SELECT * FROM user_type WHERE id = $usty";
                                                    $statement = $pdo->prepare($query);
                                                    $statement->execute();
                                                    $results = $statement->fetch(); 
                                                ?>

                                                <td><?php echo $user[$i]['user_type_id']. " - " . $results['name'];?></td>
                                                <td><a href="modificar.php?id=<?php echo $user[$i]['id']?>" class="button radius tiny secondary">Modificar</a></td>
                                                <td><a href="eliminar_usuario.php?id=<?php echo $user[$i]['id']?>" class="button radius tiny alert" onclick="alertaEliminar()">Eliminar</a></td>
                                            </tr>
                                        <?php
                                            $i++;
                                        }
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr> 
                                                ...
                                                <td><?php echo $log[$i]['user_id'] . " - " . $results['email'];?></td>
                                            </tr>
                                        <?php
                                            $i++;
                                            }
